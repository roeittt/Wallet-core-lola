...
function bech32Decode(btcAddress: string) {
  ...
  return {
    witnessVersion,
    data: base.bech32.fromWords(bech32Words.slice(1)),
  };
}
...
function bech32MDecode(btcAddress: string) {
  ...
  return {
    witnessVersion,
    data: base.bech32m.fromWords(bech32MWords.slice(1)),
  };
}
...
function _poxAddressToBtcAddress_Values(
  ...
  switch (version) {
    ...
    case PoXAddressVersion.P2WSH: {
      const words = base.bech32.toWords(hashBytes);
      return base.bech32.encode(SegwitPrefix[network], [SEGWIT_V0, ...words]);
    }
    case PoXAddressVersion.P2TR: {
      const words = base.bech32m.toWords(hashBytes);
      return base.bech32m.encode(SegwitPrefix[network], [SEGWIT_V1, ...words]);
    }
  }
