...
import { bytesToHex, concatBytes, utf8ToBytes } from '../common';
...
export function hashStructuredData(structuredData: ClarityValue): Uint8Array {
  return base.sha256(serializeCV(structuredData));
}
...
export function signStructuredData({
  ...
  const structuredDataHash: string = bytesToHex(base.sha256(encodeStructuredData({ message, domain })));
