...
export function base58CheckDecode(btcAddress: string): {
  ...
  const bytes = base.base58.decode(btcAddress);
  const payload = bytes.slice(0, -4);
  const checksum = bytes.slice(-4);
  const newChecksum = base.sha256(base.sha256(payload));
...
export function base58Encode(hash: Uint8Array) {
  const checksum = base.sha256(base.sha256(hash));
  return base.base58.encode(concatBytes(hash, checksum).slice(0, hash.length + 4));
}
