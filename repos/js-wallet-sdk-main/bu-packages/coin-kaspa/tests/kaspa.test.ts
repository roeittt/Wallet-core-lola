...
describe('kaspa', () => {
  ...
  describe('validPrivateKey edge cases', () => {
    test('validPrivateKey with various invalid formats', async () => {
      ...
      expect((await wallet.validPrivateKey({ privateKey: '' })).isValid).toBe(
        false
      );

      // Test with non-hex string
      expect(
        (await wallet.validPrivateKey({ privateKey: 'not_hex_string' })).isValid
      ).toBe(false);
  ...
  describe('validAddress edge cases', () => {
    test('validAddress with various invalid formats', async () => {
      ...
      // Test with empty string
      expect((await wallet.validAddress({ address: '' })).isValid).toBe(false);
      ...
      expect(
        (await wallet.validAddress({ address: 'kaspa:invalid' })).isValid
      ).toBe(false);
