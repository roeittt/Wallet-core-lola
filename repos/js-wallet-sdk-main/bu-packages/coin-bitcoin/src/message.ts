...
export function toCompact(i: number, signature: Uint8Array, compressed: boolean) {
    if (!(i === 0 || i === 1 || i === 2 || i === 3)) {
...
export function sign(wifPrivate: string, message: string, network?: Network | Network[], messagePrefix?: string): string {
...
export function verify(publicKey: string, message: string, sig: string, messagePrefix?: string): boolean {
    const hash = magicHash(message,messagePrefix);
