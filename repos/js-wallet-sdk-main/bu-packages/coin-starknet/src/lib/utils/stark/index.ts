...
export function randomAddress(): string {
  const randomKeyPair = signUtil.schnorr.stark.utils.randomPrivateKey();
  return signUtil.schnorr.stark.getStarkKey(randomKeyPair);
}
...
export function getFullPublicKey(privateKey: BigNumberish): string {
  ...
  const fullPrivKey = addHexPrefix(buf2hex(signUtil.schnorr.stark.getPublicKey(privKey, false)));
  return fullPrivKey;
}
