...
export function keccakBn(value: BigNumberish): string {
  ...
  return addHexPrefix(base.keccak(hexToBytes(addHexPrefix(evenHex))).toString("hex"));
}
...
function keccakHex(str: string): string {
  return addHexPrefix(base.keccak(utf8ToArray(str)).toString("hex"));
}
...
export function solidityUint256PackedKeccak256(params: BigNumberish[]): string {
  ...
  return addHexPrefix(base.toHex(base.keccak_256(hexToBytes(myEncode))));
}
