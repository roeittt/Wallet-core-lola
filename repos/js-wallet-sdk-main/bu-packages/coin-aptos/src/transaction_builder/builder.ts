...
import {TypeTagParser} from "./aptos_types";
import {DEFAULT_MAX_GAS_AMOUNT, DEFAULT_TXN_EXP_SEC_FROM_NOW, MemoizeExpiring} from "../utils";
...
export function fetchABI(modules: MoveModuleBytecode[]) {
    const abis = modules
 ...
 * @returns RawTransaction
 */
export function buildRawTransactionByABI(
    modules: MoveModuleBytecode[],
    builderConfig: ABIBuilderConfig,
    func: EntryFunctionId,
    ty_tags: MoveType[],
    args: any[],
): RawTransaction {
    ...
    const builderABI = new TransactionBuilderABI([bcsToBytes(entryFunctionABI)], builderConfig);
