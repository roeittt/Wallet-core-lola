 ...
 */
import { base, math } from "@okxweb3/coin-base"
import { arrayContentStartsWith } from "../../utils/array"
...
const pubkeyAminoPrefixSecp256k1 = base.fromHex("eb5ae987" + "21" /* fixed length */);
const pubkeyAminoPrefixEd25519 = base.fromHex("1624de64" + "20" /* fixed length */);
const pubkeyAminoPrefixSr25519 = base.fromHex("0dfb1005" + "20" /* fixed length */);
/** See https://github.com/tendermint/tendermint/commit/38b401657e4ad7a7eeb3c30a3cbf512037df3740 */
const pubkeyAminoPrefixMultisigThreshold = base.fromHex("22c1f7e2" /* variable length not included */);
...
export function decodeAminoPubkey(data: Uint8Array): Pubkey {
  if (arrayContentStartsWith(data, pubkeyAminoPrefixSecp256k1)) {
    ...
    throw new Error("Unsupported public key type. Amino data starts with: " + base.toHex(data.slice(0, 5)));
  }
}
...
export function decodeBech32Pubkey(bechEncoded: string): Pubkey {
  const [_, data] = base.fromBech32(bechEncoded);
...
function encodeUvarint(value: number | string): number[] {
  const checked = math.Uint53.fromString(value.toString()).toNumber();
  if (checked > 127) {
    throw new Error(
...
export function encodeAminoPubkey(pubkey: Pubkey): Uint8Array {
  if (isMultisigThresholdPubkey(pubkey)) {
    ...
    return new Uint8Array([...pubkeyAminoPrefixEd25519, ...base.fromBase64(pubkey.value)]);
  } else if (isSecp256k1Pubkey(pubkey)) {
    return new Uint8Array([...pubkeyAminoPrefixSecp256k1, ...base.fromBase64(pubkey.value)]);
  } else {
...
export function encodeBech32Pubkey(pubkey: Pubkey, prefix: string): string {
  return base.toBech32(prefix, encodeAminoPubkey(pubkey));
}
