 ...
 */
import { math } from '@okxweb3/coin-base';
...
export function coin(amount: number | string, denom: string): Coin {
  ...
  if (typeof amount === "number") {
    try {
      outAmount = new math.Uint53(amount).toString();
    } catch (_err) {
      throw new Error(
 ...
 *
 * ```
 * import { parseCoins } from "@cosmjs/base-signing";
 * // or
...
export function parseCoins(input: string): Coin[] {
  return input
    ...
    .map((part) => {
      ...
      return {
        amount: math.Uint64.fromString(match[1]).toString(),
        denom: match[2],
      };
    });
}
...
export function addCoins(lhs: Coin, rhs: Coin): Coin {
  ...
  return {
    amount: math.Decimal.fromAtomics(lhs.amount, 0).plus(math.Decimal.fromAtomics(rhs.amount, 0)).atomics,
    denom: lhs.denom,
  };
}
